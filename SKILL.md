...
```
api.health_check() → 返回 {token: {ok, message}, repos: {...}, all_ok}
```

### 检查步骤

1. **Token 验证**：调 `hello()` 确认 Token 有效，失败则提示用户到语雀开放平台重新生成
2. **默认知识库**：`get_repo(default_book_id)`，不存在则提示创建
3. **索引总库**（`index_books[0]`）：同上
4. **索引子库**（`index_books[1:]`）：同上
